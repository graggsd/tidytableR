...
At this point, we are ready to implement the `tidy_htmlTable` function.
Essentially, this constructs an html table using arguments similar to the 
`htmlTable` function from the `htmlTable` package.
However, whereas `htmlTable` required the user to manually
...
input data when using `tidy_htmlTable`.
